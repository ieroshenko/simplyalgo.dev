...
type MaybeMermaid =
  | { engine?: unknown; code?: unknown; title?: unknown }
  | null
  | undefined;
type MaybeReactflow =
  | {
      engine?: unknown;
      graph?: { nodes?: unknown; edges?: unknown } | unknown;
      title?: unknown;
    }
  | null
  | undefined;
...
const isMermaidDiagram = (
  d: unknown,
): d is { engine: "mermaid"; code: string; title?: string } => {
...
const isReactflowDiagram = (
  ...
  const graph = hasGraph
    ? ((r as { graph: unknown }).graph as { nodes?: unknown; edges?: unknown })
    : undefined;
...
export const useChatSession = ({
  ...
  const initializeSession = useCallback(async () => {
    ...
    try {
      ...
      const formattedMessages: ChatMessage[] = (
        ...
        suggestDiagram:
          typeof msg.suggest_diagram === "boolean"
            ? msg.suggest_diagram
            : undefined,
      }));
  ...
  const sendMessage = useCallback(
    async (content: string, options?: { action?: "diagram" }) => {
      ...
      try {
        ...
        const rawSnippets: CodeSnippet[] | undefined =
          data.codeSnippets && Array.isArray(data.codeSnippets)
            ? data.codeSnippets
            : undefined;
        ...
        let dedupedSnippets = allowSnippets
          ? dedupeSnippets(rawSnippets, messages)
          : undefined;

        // Additional guard: drop import suggestions unless the user explicitly asked about imports
        const explicitImportAsk =
          /\b(import|from\s+\w+\s+import|how\s+to\s+import)\b/i.test(
            lastUserMsg,
          );
        if (dedupedSnippets && !explicitImportAsk) {
          dedupedSnippets = dedupedSnippets.filter((s) => {
            const isImport =
              s.insertionHint?.type === "import" ||
              /^(\s*)(from\s+\S+\s+import\s+\S+|import\s+\S+)/.test(
                s.code || "",
              );
            return !isImport;
          });
        ...
        const aiResponse: ChatMessage = {
          ...
          suggestDiagram:
            typeof data.suggestDiagram === "boolean"
              ? data.suggestDiagram
              : undefined,
        };
    ...
    async (sourceText: string) => {
      ...
      try {
        ...
        if (!diagramPayload) {
          toast({
            title: "No diagram",
            description: "The model did not return a diagram for this request.",
          });
          return;
        }
