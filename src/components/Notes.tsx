...
import { Trash2, Save, FileCheck } from 'lucide-react';
...
const Notes = ({ problemId }: NotesProps) => {
  ...
  const saveNotes = useCallback(async (noteContent: string) => {
    if (!user || !problemId) return;
        ...
        <div className="flex items-center space-x-4">
          ...
          {!isSaving && lastSaved && !hasUnsavedChanges && (
            <div className="flex items-center text-emerald-600 dark:text-emerald-400">
              ...
              <span className="text-xs font-medium">Saved {lastSaved.toLocaleTimeString()}</span>
            </div>
          )}
          {hasUnsavedChanges && !isSaving && (
            <div className="text-xs text-amber-600 dark:text-amber-400">Unsaved changes</div>
          )}
        </div>
